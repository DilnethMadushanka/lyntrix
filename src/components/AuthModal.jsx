...
import { User, Mail, Lock, Building, Calendar, Phone, Globe, ShieldCheck, X, Sparkles, AlertCircle, ArrowRight } from 'lucide-react';
...
export default function AuthModal({ isOpen, onClose, onAuthSuccess }) {
  ...
  const handleGoogleAuth = () => {
    ...
    setTimeout(() => {
      try {
        // Simulate Google OAuth 2.0 Identity Token exchange
        const googleProfile = {
          name: name || 'Dilneth Madushanka',
          email: email || 'dilneth.google@lyntrix.tech',
          company: company || 'Lyntrix Enterprise Client',
          birthday: birthday || '1998-05-14',
        ...
        setLoading(false);
        setError('Google Single Sign-In failed.');
      }
    }, 800);
  };
      ...
      <div className="relative w-full max-w-lg glass-card p-6 sm:p-8 rounded-2xl border border-cyan-500/40 shadow-2xl shadow-cyan-950 max-h-[90vh] overflow-y-auto">
        ...
        <div className="text-center space-y-2 mb-6">
          ...
          <h3 className="text-2xl font-extrabold text-white font-['Outfit']">
            {mode === 'signin' ? 'Client Portal Sign In' : 'Create Enterprise Account'}
          </h3>
          <p className="text-xs text-slate-400 font-mono">
            {mode === 'signin' ? 'ACCESS YOUR PROJECTS & PROPOSALS' : 'REGISTER COMPANY PROFILE & SCOPE'}
          </p>
        </div>
        ...
        <div className="mb-5">
          ...
          <div className="relative my-4">
            ...
            <div className="relative flex justify-center text-[10px] uppercase font-mono"><span className="bg-[#0d111a] px-2 text-slate-500">OR EMAIL LOGIN</span></div>
          </div>
        </div>
